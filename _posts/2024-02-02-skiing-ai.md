...
If you have ever been skiing you surely explored the app store in search of an app that records your day on the slope. Today, there are lots of choices for apps logging the route, top speeds and the total distance travelled. Let’s delve into how they actually work and uncover the magic behind the scenes that provides metrics such as the difference in time spent on lifts and pistes.
...
Most often, the simplest solutions work best and as such I wanted to give a simple perceptron a shot at classifying our data first. My initial attempt at building a simple neural network looked like this in tensorflow:
...
The best results with the convolutional network were achieved with `400` datapoints, four times more than what the simple perceptron performed best with. This tradeoff improved performance by 5%, which might make it worth it.
...
To get a feeling for where our model was hitting it’s limits, I wanted to visualise the location data and the corresponding prediction that the model made. A red point corresponds to what the model predicts is lift, a green point corresponds to piste.
...
However, I still wanted to try to get the model to improve it’s accuracy, hoping that more training and optimisation might make it possible to distinguish standing still from sitting on a lift.
...
Now, if you look at the incorrectly predicted sections (red sections), the color on the right map is dark, meaning the speed was relatively slow and more important, the speed is very constant over these sections.
...
If we look at this map, which shows the rate of change in altitude (how much I went up and down between the measurements) you can see that the incorrectly labelled sections are mostly straight horizontal sections, because the color is dark (meaning low rate of change of altitude). This further corroborates the hypothesis above.
...
Doesn’t this seem a bit over the top for something intuition tells us, a naïve algorithm could handle? Well let’s try to actually build that algorithm.
...
It also only took me 15 minutes to figure this out, instead of the weeks spent optimising the model.
